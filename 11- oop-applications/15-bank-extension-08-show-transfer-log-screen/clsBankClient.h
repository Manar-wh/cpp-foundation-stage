...
class clsBankClient : public clsPerson
{
	...
	static stTransferLogRecord _ConvertTransferLineToRecord(string Line, string Seperator = "#//#") {
		...
		vector <string> vData = clsStrings::Split(Line, Seperator);
		stTransferLogRecord TransferRec;
		TransferRec.DateTime = vData[0];
		TransferRec.SourceAccNum = vData[1];
		TransferRec.DestinationAccNum = vData[2];
		TransferRec.Amount = stod(vData[3]);
		TransferRec.SourceBalance = stod(vData[4]);
		TransferRec.DestinationBalance = stod(vData[5]);
		TransferRec.Username = vData[6];
		return TransferRec;
	}
	...
	string _ConvertObjectToLine(clsBankClient Client,string Delim= "#//#") {
	 ...
	 string _ConvertTransferDataToLine(clsBankClient Client2, double Amount,string Delim = "#//#") {
		string DataLine;
		DataLine += clsDate::GetSystemDateTimeString() + Delim;
		DataLine += getAccountNumber() + Delim;
		DataLine += Client2.AccountNumber + Delim;
		DataLine += to_string(Amount) + Delim;
		DataLine += to_string(getAccountBalance()) + Delim;
		DataLine += to_string(Client2.AccountBalance) + Delim;
...
public:
 ...
 enum enSaveResult{svFailedEmptyObject = 0 , svSucceeded =1, svFailedAccountNumberExist = 2};
	...
	static int TotalBalances() {
		int Total = 0;
		vector <clsBankClient> vAllClients = _LoadDataFromFileToVector();
		for (clsBankClient &C : vAllClients)
		{
	...
	bool Transfer(double Amount, clsBankClient &DestinaionClient) {
		...
		Withdraw(Amount);
		DestinaionClient.Deposit(Amount);

		RegisterTransferLog(_ConvertTransferDataToLine(DestinaionClient, Amount));
		return true;
	}

	static void RegisterTransferLog(string DataLine) {
		fstream MyFile;
	...
	static vector <stTransferLogRecord> GetTransferLogRecord() {
		...
		if (MyFile.is_open())
		{
			...
			while (getline(MyFile, Line))
			{
				vTransferRecord.push_back(_ConvertTransferLineToRecord(Line));
			}
			MyFile.close();
		}
