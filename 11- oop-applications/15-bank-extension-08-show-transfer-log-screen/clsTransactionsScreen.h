...
class clsTransactionsScreen : protected clsScreen
{
	enum enTransactions 
	{
		DepositOp = 1, WithdrawOp = 2, TotalBalancesOp = 3, TransferOp = 4 , TransferLogOp = 5, MainMenuOp = 6
	};

	static short _ReadTransactionsOption() {
		cout << setw(37) << left << "" << "Please Choose What You Want To Do? [1 to 6]:  ";
		short Option = 0;
	...
	static void _performeMainMenuoptions(enTransactions Option) {
		...
		case clsTransactionsScreen::TransferOp:
			system("cls");
			_ShowTransferScreen();
			_GoBackToTransactionsMenu();
			break;

		case clsTransactionsScreen::TransferLogOp:
			system("cls");
