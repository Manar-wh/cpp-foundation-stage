...
public:
	...
	static clsCurrency FindByCode(string CurrencyCode) {
		...
		if (MyFile.is_open())
		{
			...
			{
				clsCurrency Currency = _ConvertLineToCurrencyObject(Line);
				if (clsStrings::ConvertAllLettersToUpper(Currency.CurrencyCode())== CurrencyCode)
				{
					MyFile.close();
					return Currency;
				}
	...
	float CurrencyCalculator(clsCurrency Currency2, float Amount) {

		 float ToUSD = ConvertToUSD(Amount);

		 if (clsStrings::ConvertAllLettersToUpper(Currency2.CurrencyCode()) == ("USD"))
		 {
			 return ToUSD;
		 }

		return ToUSD * Currency2.Rate();
	}
};
