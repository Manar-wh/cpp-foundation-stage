# 🚀 C++ First Foundation Stage (Course 01-12)

This repository documents my journey through the **First Foundation Stage** of the roadmap provided by **Dr. Mohammed Abu-Hadhoud** (Programming Advices). 

The goal of this stage is to build a rock-solid foundation in logic, problem-solving, and C++ programming.

---

## 📑 Roadmap & Progress

### 🏗️ Phase 1: Fundamentals (Completed ✅)
*These courses focused on building the basic programming mindset and logic.*
- [x] **Course 01:** Programming Advices - Introduction
- [x] **Course 02:** Introduction to Programming
- [x] **Course 03:** Algorithms & Problem Solving Level 1
- [x] **Course 04:** C++ Level 1
- [x] **Course 05:** Algorithms & Problem Solving Level 2

### 💻 Phase 2: Intermediate Concepts (Currently Uploading 🛠️)
*The following courses contain practical exercises and solutions:*
- [x] **Course 06:** C++ Level 2 
- [ ] **Course 07:** Algorithms & Problem Solving Level 3 (In Progress)
- [ ] **Course 08:** Strings & Dates
- [ ] **Course 09:** Memory Management & Pointers
- [ ] **Course 10:** Functional Programming & Logic
- [ ] **Course 11:** OOP Level 1
- [ ] **Course 12:** Data Structures Level 1

---

## 🛠️ Main Skills Covered
* **Advanced Logic Building:** Solving complex algorithmic problems.
* **Data Organization:** Mastering Vectors, Arrays, and Structures.
* **Clean Code:** Writing readable and maintainable C++ code.

---
*“Success is the sum of small efforts, repeated day in and day out.”*
